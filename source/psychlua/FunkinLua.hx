...
import shaders.Shaders.ChromaticAberrationEffect;
...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		Lua_helper.add_callback(lua, "setAntialiasing", function(obj:String, ANT:Bool = true) {
			// Verifica si el objeto existe en el juego
			if (game.getLuaObject(obj) != null) {
				var sprite:FlxSprite = game.getLuaObject(obj); // Obtiene el objeto FlxSprite
				sprite.antialiasing = ANT; // Establece la propiedad antialiasing del objeto FlxSprite
				return; // Sale de la función
			}
		
			var split:Array<String> = obj.split('.');
			var sprite:FlxSprite = LuaUtils.getObjectDirectly(split[0]);
		
			if (split.length > 1) {
				sprite = LuaUtils.getVarInArray(LuaUtils.getPropertyLoop(split), split[split.length - 1]); // Busca el objeto FlxSprite utilizando propiedades anidadas
			}
		
			// Si se encuentra el objeto, establece la propiedad antialiasing
			if (sprite != null) {
				sprite.antialiasing = ANT;
				return; // Sale de la función
			}
			
			// Si no se encontró el objeto, muestra un mensaje de error
			luaTrace('setAntialiasing: Could not find object: ' + obj, false, false, FlxColor.RED);
		});
		...
		//shaders
		if (ClientPrefs.data.Luashaders == true)
			{
				Lua_helper.add_callback(lua, "doBloomPulse", function(camera:String, intensityFrom:Float = 0.5, length:Float = 0.25, ease:String = "linear")
					{
						var bloom:BloomEffect2 = null; // to shut haxe up
						switch (camera.toLowerCase())
						{
							case 'camhud' | 'hud':
								for (effect in PlayState.instance.camHUDShaders)
								{
									if (Std.isOfType(effect, BloomEffect2))
									{
										bloom = effect;
										break;
									}
							...
							default:
								if (PlayState.instance.modchartSprites.exists(camera))
								{
									...
									if (Std.isOfType(shaderObj, BloomEffect2))
										bloom = shaderObj;
								}
						...
						FlxTween.tween(bloom, {intensity: 0}, length, { ease: LuaUtils.getTweenEaseByString(ease) });
					});
					Lua_helper.add_callback(lua, "doChromaticPulse", function(camera:String, newOffset:Float = 0.03, length:Float = 0.25, ease:String = "linear") {
						var googlechrom:DoChromaticAberrationEffect = null;
					
						// Buscar el efecto de aberración cromática
						switch (camera.toLowerCase()) {
							case 'camhud', 'hud':
								for (effect in PlayState.instance.camHUDShaders) {
									if (Std.isOfType(effect, DoChromaticAberrationEffect)) {
										googlechrom = effect;
									}
								}
							...
							default:
								if (PlayState.instance.modchartSprites.exists(camera)) {
									var shaderObj = Reflect.getProperty(PlayState.instance.modchartSprites.get(camera), "shader");
									if (Std.isOfType(shaderObj, DoChromaticAberrationEffect)) {
						...
						// Inicializar el efecto de aberración cromática si no se encontró
						if (googlechrom == null) {
							googlechrom = new DoChromaticAberrationEffect();
							switch (camera.toLowerCase()) {
								case 'camhud', 'hud':
									PlayState.instance.camHUDShaders.push(googlechrom);
								case 'camother', 'other':
									PlayState.instance.camOtherShaders.push(googlechrom);
								case 'camgame', 'game':
									PlayState.instance.camGameShaders.push(googlechrom);
								default:
									if (PlayState.instance.modchartSprites.exists(camera)) {
										var shaderObj = Reflect.getProperty(PlayState.instance.modchartSprites.get(camera), "shader");
										if (Std.isOfType(shaderObj, DoChromaticAberrationEffect)) {
											shaderObj.push(googlechrom);
										}
									} else if (PlayState.instance.modchartTexts.exists(camera)) {
										var shaderObj = Reflect.getProperty(PlayState.instance.modchartTexts.get(camera), "shader");
										shaderObj.push(googlechrom);
						...
						// Configurar el valor del offset
						googlechrom.offset = newOffset;
					
						// Realizar la transición
						FlxTween.tween(googlechrom, {offset: 0.0}, length, { ease: LuaUtils.getTweenEaseByString(ease) });
					});
				...
				Lua_helper.add_callback(lua, "addBarrelBlurAbberationEffect", function(camera:String,barrel:Float = 1.0,zoom:Float = 1.0,doChroma:Bool = false,angle:Float = 0.0,x:Float = 0.0,y:Float = 0.0) {
					
					PlayState.instance.addShaderToCamera(camera, new BarrelBlurEffect(barrel,zoom,doChroma,angle,x,y));
					
				});
				Lua_helper.add_callback(lua, "addBlueChromaticAbberationEffect", function(camera:String,strength:Float = 0.005) {
				...
				Lua_helper.add_callback(lua, "addSobelEffect", function(camera:String,strength:Float = 0.1,intensity:Float = 0.1) {
					
					PlayState.instance.addShaderToCamera(camera, new SobelEffect(strength,intensity));
					
				});
				Lua_helper.add_callback(lua, "addRainEffect", function(camera:String,iTime:Float = 20) {
					
					PlayState.instance.addShaderToCamera(camera, new RainEffect(iTime));
					
				});
				Lua_helper.add_callback(lua, "addMirrorRepeatEffect", function(camera:String,zoom:Float = 1, angle:Float = 1, iTime:Float = 1, x:Float = 1, y:Float = 1) {
					
					PlayState.instance.addShaderToCamera(camera, new MirrorRepeatEffect(zoom,angle,iTime,x,y));
					
				});
				Lua_helper.add_callback(lua, "addHeatEffect", function(camera:String,strength:Float,iTime:Float) {
					
					PlayState.instance.addShaderToCamera(camera, new HeatEffect(strength,iTime));
					
				});
				Lua_helper.add_callback(lua, "addGaussianBlurEffect", function(camera:String,loops:Float= 16.0,quality:Float = 5.0,strength:Float = 20.0) {
					
					PlayState.instance.addShaderToCamera(camera, new BetterBlurEffect(loops,quality,strength));
					
				});
				Lua_helper.add_callback(lua, "addScanlineEffect", function(camera:String,strength:Float = 0.0,pixelsBetweenEachLine:Float = 15.0,smooth:Bool = false) {
					
					PlayState.instance.addShaderToCamera(camera, new ScanlineEffect(strength,pixelsBetweenEachLine,smooth));
					
				});
				Lua_helper.add_callback(lua, "addScanlineEffect2", function(camera:String,lockAlpha:Bool = false) {
					
					PlayState.instance.addShaderToCamera(camera, new ScanlineEffect2(lockAlpha));
					
				});
				Lua_helper.add_callback(lua, "addVignetteEffect", function(camera:String,strength:Float = 1.0,size:Float = 0.0,red:Float = 0.0,green:Float = 0.0,blue:Float = 0.0) {
					
					PlayState.instance.addShaderToCamera(camera, new VignetteEffect(strength,size,red,green,blue));
					
				});
				Lua_helper.add_callback(lua, "addDistortedNoiseEffect", function(camera:String,strength:Float = 1) {
					
					PlayState.instance.addShaderToCamera(camera, new WaveBurstEffect(strength));
					
				});
				Lua_helper.add_callback(lua, "addPerlinSmokeEffect", function(camera:String,waveStrength:Float = 1,smokeStrength:Float = 1) {
					
					PlayState.instance.addShaderToCamera(camera, new PerlinSmokeEffect(waveStrength,smokeStrength));
				...
				Lua_helper.add_callback(lua, "addBlockedEffect", function(camera:String,res:Float, time:Float, colorMultiplier:Float, colorTransform:Bool) {
				...
				Lua_helper.add_callback(lua, "addBloomEffect", function(camera:String,effect:Float = 5,strength:Float = 0.2,contrast:Float = 1.0,brightness:Float = 0.0) {
			
					PlayState.instance.addShaderToCamera(camera, new BloomEffect(effect,strength,contrast,brightness));
					
				});
				Lua_helper.add_callback(lua, "addBloomEffect2", function(camera:String,intensity:Float = 0.35,blurSize:Float=1.0) {
			
					PlayState.instance.addShaderToCamera(camera, new BloomEffect2(blurSize/512.0,intensity));
					
				});
				Lua_helper.add_callback(lua, "clearEffects", function(camera:String) {
					PlayState.instance.clearShaderFromCamera(camera);
				});
				Lua_helper.add_callback(lua, "removeEffect", function(camera:String, effect:ShaderEffect)
				{
