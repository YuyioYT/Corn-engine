...
class CreditsEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		background = new FlxSprite().loadGraphic(Paths.image('menuBG/menuDesat'));
		add(background);
	...
	function updateCreditObjects(){
		...
		for (i in 0...creditsStuff.length)
		{
			...
			if(isSelectable) {
				...
				var icon:HealthIcon;
				if(Paths.fileExists('images/credits/' + creditsStuff[i][1] + '.png', IMAGE)) icon = new HealthIcon('credits/' + creditsStuff[i][1]);
				else {
					icon = new HealthIcon('credits/unknownIcon'); // If icon didnt load it will load the unknow icon.
					if(creditsStuff[i][1] == null || creditsStuff[i][1] == '') icon = new HealthIcon('credits/unknownIcon');
				}

				icon.x = optionText.width + 10;
				icon.y = optionText.width + 10;
	...
	function changeSelection(change:Int = 0, playSound:Bool = true)
	{
		...
		if(change != 0){
			...
			moveTween = FlxTween.tween(descriptionText, {y : descriptionText.y + 75}, 0.9, {ease: FlxEase.sineOut});
		}
