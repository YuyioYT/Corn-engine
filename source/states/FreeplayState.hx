...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		gradientBar = FlxGradient.createGradientFlxSprite(Math.round(FlxG.width), 512, [0x00ff0000, 0x2AFFFFFF, 0x75FFFFFF], 1, 90, true);
		...
		for (i in 0...songs.length)
		{
			...
			icon.animation.addByPrefix('Neutral', 'Neutral', HealthIcon.framerate, true);
			icon.animation.addByPrefix('Defeat', 'Defeat', HealthIcon.framerate, true);
			icon.animation.addByPrefix('Winning', 'Winning', HealthIcon.framerate, true);
		...
		scoreText.setFormat(Paths.font(typografy), 32, FlxColor.WHITE, RIGHT);

		scoreBG = new FlxSprite(scoreText.x - 6, 0).makeGraphic(1, 66, 0xFF8F4500);
		scoreBG.alpha = 0.6;
		add(scoreBG);
		...
		text.setFormat(Paths.font(typografy), size, FlxColor.WHITE, RIGHT);
	...
	function changeSelection(change:Int = 0, playSound:Bool = true)
	{
		...
		if (iconArray[curSelected].animation.curAnim != null)
		{
			...
			if (ClientPrefs.data.iconChangeonFreeplay)
			{
				...
				iconArray[curSelected].animation.play('Defeat');
			}
		}
